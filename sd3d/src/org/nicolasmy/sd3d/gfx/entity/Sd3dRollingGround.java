...
public class Sd3dRollingGround extends Sd3dGameEntity {
	...
	public Sd3dRollingGround(float tileSize, float height)
	{
		mCloudLength = 2;
		mSize = tileSize;
		...
		for (int i = 0;i < mCloudLength;i++)
			for (int j = 0;j < mCloudLength;j++)
			{
				...
				setupCloudTile(i*mSize - middleX, height,j*mSize - middleY,mSize,"textures/road.png",
		...
		mClipingBottomY = - middleY;
		mClipingTopX = mCloudLength * mSize  - middleX;
		mClipingTopY = mCloudLength * mSize - middleY;
		mVelocity[2] = 20.f;
	}
	...
	public void onProcessFrame(int elapsedtime)
	{
		...
		for (int i = 0;i < mObject.mMesh.length;i++)
		{
			...
			if (this.mObject.mMesh[i].mMeshPosition[2] > mClipingTopY)
